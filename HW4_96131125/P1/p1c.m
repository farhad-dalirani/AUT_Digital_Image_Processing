...
for filt_size_i = 1:3
    ...
    % Apply filter (Harmonic Mean Filter) 
    for i=(point1(1,1)):(point2(1,1))
        for j=(point1(1,2)):(point2(1,2))
            ...
            filter_i_j =  (size(neighborhood,1)*size(neighborhood,2))/...
                (sum(sum(neighborhood.^(-1), 1),2));
    ...
    imwrite(filtered_imgage, strcat('p1c_', num2str(filt_size),'.png'))
end