...
for i = 1:size(balls_in_group,1)
   ...
   imwrite(img_rgb .* repmat(balls_grouped(:,:,i),[1,1,3]), ...
       strcat('p8c-Group-', num2str(i),'Number of balls ', num2str(number_of_balls_in_group(1,i)),'.png'))
end

count_brand = 0;
addidas_brand = zeros(size(img_rgb, 1), size(img_rgb, 2));
for i=1:size(balls_in_group,1)
   if group_brand(1,i) == 1
      ...
      addidas_brand = addidas_brand | balls_grouped(:,:,i);
   end
end

figure();
   imshow(img_rgb .* repmat(addidas_brand,[1,1,3]));
title(strcat('Addidas Brand-','Number of balls:', num2str(count_brand)));
imwrite(img_rgb .* repmat(addidas_brand,[1,1,3]), ...
       strcat('p8c-Addidas Brand-','Number of balls ', num2str(count_brand),'.png'))

disp('Number of balls of each type');
disp(number_of_balls_in_group)
disp('Number of ball which belong to Adidas')
disp(count_brand)