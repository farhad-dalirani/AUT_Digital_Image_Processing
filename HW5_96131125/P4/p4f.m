...
% read modified image
img_morf = imread('viva_team_melli_f.png');
...
for i = 1:511
   ...
   img_dilate = imdilate(img_org, se);
    
   cur_mse = (1/9)*sum(sum(((double(img_dilate)-double(img_morf)).^2)));
...
img_dilate = imdilate(img_org, matched_se);
