...
% read modified image
img_morf = imread('viva_team_melli_d.png');
...
for i = 1:511
   ...
   img_erode = imerode(img_org, se);
    
   cur_mse = (1/9)*sum(sum(((double(img_erode)-double(img_morf)).^2)));
...
img_erode = imerode(img_org, matched_se);
