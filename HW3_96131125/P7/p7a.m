% Read Image
img_org = imread('little_donald.png');
figure();
imshow(img_org);
title('Input Image');
...
%title('Spectrom')
figure();
imshow(log10(img_spectrom/1000+1))
title('Spectrum')
figure();
imshow(img_phase)
title('Phase')
imwrite(log10(img_spectrom/1000+1), 'p7a_spectrum.png');
...
y_count = 0;
locs = [];
for p = 76:139:size(img_F,1)
    x_count = x_count+1;
    for q = 54:112:floor(size(img_F,2)/2)+30
        y_count = y_count+1;
